...
Form::Form(const std::string& _name, const int& _require, const int& _execute) : name(_name), sign(0), require(_require), execute(_execute)
{
	try
	{
...
Form::Form(const Form& f) : name(f.getName()), sign(f.getSigned()), require(f.getRequire()), execute(f.getExecute())
{
