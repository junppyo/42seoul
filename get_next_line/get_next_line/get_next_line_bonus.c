...
/*   Updated: 2022/04/04 14:09:50 by junpkim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load(char **ret, char **save)
{
	...
	if (*save == NULL)
		return ;
	t = chk_str(*save);
	if (t != -1 && t != (int)ft_strlen(*save))
	{
		*ret = make_str(*ret, *save, t);
		tmp = make_str(NULL, *save + t, ft_strlen(*save + t));
		free(*save);
		*save = NULL;
		if (tmp != NULL)
			*save = tmp;
...
void	read_fd(char **save, char **ret, int fd)
{
	...
	while (n > 0)
	{
		...
		if (tmp != -1)
		{
			*save = make_str(*save, &buf[tmp], ft_strlen(&buf[tmp]));
